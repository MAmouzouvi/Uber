        ...
        nLogin.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (TextUtils.isEmpty(email)){
                    Toast.makeText(DriverLoginActivity.this,"Please Enter your email address", Toast.LENGTH_LONG).show();
                    nEmail.setError("Email is required");
                    nEmail.requestFocus();

                }
                else if(!Patterns.EMAIL_ADDRESS.matcher(email).matches()){
                    Toast.makeText(DriverLoginActivity.this,"Please re-enter your email", Toast.LENGTH_LONG).show();
                    nEmail.setError("Valid email is required");
                    nEmail.requestFocus();


                } else if (TextUtils.isEmpty(password)){
                    Toast.makeText(DriverLoginActivity.this,"Please Enter your password", Toast.LENGTH_LONG).show();
                    nPassword.setError("Password is required");
                    nPassword.requestFocus();

                } else if(password.length() <= 5){
                    Toast.makeText(DriverLoginActivity.this,"Please a longer", Toast.LENGTH_LONG).show();
...
//        });
    ...
    private void LoginUser(String email, String password) {
        ...
        authProfile.signInWithEmailAndPassword(email,password).addOnCompleteListener(new OnCompleteListener<AuthResult>() {
            ...
            public void onComplete(@NonNull Task<AuthResult> task) {
                if(task.isSuccessful()){
                    Toast.makeText(DriverLoginActivity.this,"You are logged in now", Toast.LENGTH_LONG).show();
